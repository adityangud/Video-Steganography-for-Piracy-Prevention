...
for step in xrange(300):
    ...
    kernel_code = open("embed_0.cl").read() % (new_dim[1], new_dim[2], new_dim[3])
    prg1 = cl.Program(ctx, kernel_code).build()

    stime = time.time()
    prg1.embed_zero(queue, (new_dim[0], new_dim[1], new_dim[2]) , None, a_buf, dest_buf)
    etime = time.time()
